...
import { Agent, createV4CompatibleResponse } from '@mastra/core/agent';
...
const mbtiAgent = new Agent({
  ...
  1.  If this is the start of the conversation (no conversation history), your first response must be ONLY the following, with each language on a new line:
      ...
      Hola, soy un chatbot de diagn\xF3stico MBTI. \xBFQu\xE9 idioma te gustar\xEDa usar?
...
async function streamGenerateHandler(c2) {
  try {
    ...
    const runtimeContext = c2.get("runtimeContext");
    const body = await c2.req.json();
    const clientSdkCompat = c2.req.header("x-ai-sdk-compat");
...
async function getMessagesHandler(c2) {
  try {
    ...
    const networkId = c2.req.query("networkId");
    const threadId = c2.req.param("threadId");
    const format = c2.req.query("format");
